...
app.config['UPLOAD_FOLDER'] = VIDEOS_FOLDER
...
def serve_video(filename):
    ...
    response = send_from_directory(VIDEOS_FOLDER, filename)
...
def upload():
    ...
    ext = original_filename.rsplit('.', 1)[1].upper()
    ...
    return jsonify({'success': True, 'message': f'"{original_filename}" uploaded!'})
...
def like(video_id):
    ...
    r2 = requests.get(
        f'{SUPABASE_URL}/rest/v1/likes?video_id=eq.{video_id}',
        headers=supabase_headers()
    )
